...
ใช้ **GitHub Actions** รันอัตโนมัติทุกครั้งที่เปิด Pull Request เข้า
`develop` หรือ `main`:
...
jobs:
  ...
  - Unit Test        (รัน Test Suite ที่เขียนไว้ — ดู TEST_PLAN.md)
  - Build Check      (ตรวจว่า Build ผ่านจริง ไม่ใช่แค่รันเครื่องได้)
```
...
เขียน Deploy Script เอง — Railway ตรวจ Push เข้า `develop`/`main` แล้ว
