...
| Field | Type | บังคับ | คำอธิบาย |
|---|---|---|---|
| `amountTotal` | number | ⚠️ | **จำนวนเงินรวม** (ไม่ใช่จำนวนหน่วย) > 0 — หน่วยตาม `currency` · **บังคับ ยกเว้น**เมื่อส่ง `quantity` + `pricePerUnit` มาครบ (ยอดรวมคำนวณจากคู่นั้นแทน) |
| `quantity` | number | — | **จำนวนหน่วยที่ได้จริง** — ส่งคู่กับ `pricePerUnit` เมื่อรู้ตัวเลขจริง (เช่นอ่านจากสลิป) → Backend บันทึกตามนี้ตรงๆ **ไม่ดึงราคาตลาดมาคำนวณใหม่** · ส่งมาลำพังไม่ได้ (ต้องมี `pricePerUnit` ด้วย) |
...
|---|---|---|
| **ส่ง `quantity` + `pricePerUnit` ครบ** | **บังคับ** | ⭐ **ใช้ค่าที่ส่งมาตรงๆ ไม่แตะ Price Feed เลย** (`priceSource: 'user'`) — เส้นทางสำหรับรายการที่รู้ตัวเลขจริง เช่นอ่านจากสลิป |
...
> ⚠️ **ทำไมต้องมีเส้นทางแรก:** สลิปคือหลักฐานของสิ่งที่เกิดขึ้นจริง ส่วนราคาตลาดเป็น
> เพียงตัวประมาณสำหรับกรณีที่ไม่มีข้อมูลจริง — รายการที่บันทึกย้อนหลัง (สลิปเก่า)
> ถ้าเอายอดเงินไปหารด้วย "ราคาตลาด ณ ตอนกดบันทึก" จะได้จำนวนหน่วยและต้นทุนที่
> **ไม่ตรงกับสลิปเลย** และยิ่งสลิปเก่ายิ่งเพี้ยนมาก (เคสจริง: สลิป ASTS 12 ส.ค.
> `20.0104114 @ 74.84 USD` บันทึกวันที่ 22 ส.ค.) — เป็น Immutable Ledger ที่แก้
> ย้อนหลังได้ด้วย Reversal เท่านั้น
...
## 15.8 POST `/api/v1/transactions/slip-ocr` — ให้ AI อ่านสลิป (ไม่บันทึก)

อัปโหลดรูปสลิปแล้วให้ Claude Vision อ่านค่าออกมา **คืนค่าเฉยๆ ไม่สร้างธุรกรรม**
— Frontend เอาไป Prefill ฟอร์มให้ผู้ใช้ **ตรวจ + แก้ไข + กดยืนยันเอง** แล้วค่อยเรียก
§ 15.2 ตามปกติ (Pattern เดียวกับการ์ด Preview ใน LINE — ไม่มีทางที่ค่าจาก AI ถูก
บันทึกลง Ledger โดยไม่ผ่านการยืนยัน)

**Request** — Binary รูปภาพดิบ (`express.raw` เหมือน `POST /:id/slip`)
`Content-Type` = ชนิดรูปจริง (`image/jpeg` / `png` / `webp` / `gif`) · สูงสุด 10 MB

**สิทธิ์การใช้งาน** — ตัดสินที่ `slipOcrAccess.service` (ตัวเดียวกับฝั่ง LINE):

| ผู้ใช้ | เงื่อนไข |
|---|---|
| Premium | ผ่านเสมอ — โควตารายเดือน 50 ครั้งบังคับใน `slipOcr.service` เหมือนเดิม |
| Free | ทดลองฟรี **3 ครั้งตลอดอายุบัญชี** (ไม่ใช่ต่อเดือน) + เพดานเรียก Claude 12 ครั้ง |

> ⚠️ **โควตาเป็นถังเดียวกับ LINE** — นับจาก `ai_ocr_usage` ตาราง/คอลัมน์เดียวกัน
> (`count` + `call_count`) ใช้ทางเว็บแล้วโควตาฝั่ง LINE ลดตามทันที และกลับกัน
> ห้ามสร้างตัวนับใหม่แยกเด็ดขาด (มี Regression Test คุมไว้)

**Response 200**
```json
{
  "slip": {
    "symbol": "BTC", "side": "buy", "orderStatus": "filled",
    "quantity": 0.01, "pricePerUnit": 3400000, "amountTotal": 34000,
    "currency": "THB", "date": "2026-08-20", "confidence": "high"
  },
  "slipToken": "1750000000000.jpg",
  "quota": { "mode": "premium", "remaining": 49 }
}
```

| Field | คำอธิบาย |
|---|---|
| `slip.side` | `"buy"` \| `"sell"` \| **`null`** — `null` = อ่านไม่ชัด/สัญญาณขัดกัน **Frontend ต้องบังคับให้ผู้ใช้เลือกเอง ห้ามเดา** |
| `slip.orderStatus` | `"filled"` \| `"pending"` \| `"cancelled"` \| `null` — `pending`/`cancelled` = คำสั่งยังไม่เกิดขึ้นจริง **ห้าม Prefill ให้กดบันทึกง่ายๆ** |
| `slipToken` | Token รูปที่เก็บไว้ ส่งต่อให้ § 15.2 ตอนยืนยัน · **`null` สำหรับผู้ใช้ทดลองฟรี** (การเก็บรูปเป็นหลักฐานเป็นสิทธิ์ Premium แยกต่างหาก) |
| `quota.mode` | `"premium"` (remaining = รายเดือน) \| `"trial"` (remaining = คงเหลือตลอดอายุบัญชี) |

**Error Codes (§15.8)**

| Code | HTTP | ความหมาย | นับโควตา? |
|---|---|---|---|
| `OCR_PREMIUM_REQUIRED` | 403 | ไม่ใช่ Premium และไม่เคยมีสิทธิ์ทดลอง | ไม่ |
| `OCR_TRIAL_EXHAUSTED` | 403 | ใช้สิทธิ์ทดลองฟรีครบแล้ว | ไม่ |
| `OCR_QUOTA_EXCEEDED` | 429 | โควตารายเดือนของ Premium เต็ม | ไม่ |
| `OCR_CALL_LIMIT_EXCEEDED` | 429 | ชนเพดานคุมต้นทุน (เรียก Claude จริง) | ไม่ |
| `OCR_RATE_LIMITED` | 429 | ส่งถี่เกิน 1 ครั้ง/10 วินาที | ไม่ |
| `OCR_NOT_A_SLIP` | 422 | ไม่ใช่สลิป / อ่าน symbol ไม่ออก | **ไม่** (แต่ `call_count` +1) |
| `OCR_MULTIPLE_ITEMS` | 422 | มีหลายรายการในรูปเดียว | **ไม่** (แต่ `call_count` +1) |
| `OCR_FAILED` | 502 | Claude ล่ม/Timeout/ตอบไม่ใช่ JSON | **ไม่** (แต่ `call_count` +1) |
| `OCR_NOT_CONFIGURED` | 503 | ยังไม่ได้ตั้ง `CLAUDE_API_KEY` | ไม่ |
| `INVALID_SLIP_CONTENT_TYPE` | 415 | ไม่ใช่ไฟล์รูป | ไม่ |
| `SLIP_TOO_LARGE` | 413 | เกิน 10 MB | ไม่ |

---

## 15.9 LINE — ถามหาสลิปหลังบันทึกด้วยการพิมพ์

ผู้ใช้ที่ **พิมพ์เอง** (`"ซื้อ ASTS 5000"` → กดยืนยัน) เดิมไม่มีโอกาสแนบสลิปเลย
(ระบบแนบให้อัตโนมัติเฉพาะเส้นทาง AI OCR ที่มีรูปอยู่ในมือตั้งแต่แรก)

**Flow:** กดยืนยัน → บันทึกสำเร็จ → เปิด Session (`transaction_slip_sessions`,
TTL **10 นาที**) + ส่งการ์ดชวนแนบสลิป → ผู้ใช้ส่งรูป → แนบเข้ารายการนั้น (**ไม่เรียก
AI ไม่กินโควตา**) → ลบ Session

**ลำดับการตัดสินใน `handleImage`** (สำคัญ — ห้ามสลับ):

1. มีคำขอชำระเงิน pending → สลิปโอนเงิน Premium (Flow เดิม Round 5)
2. **มี Session รอสลิป → แนบเข้ารายการนั้น** (ใหม่ — ไม่เรียก Claude)
3. ไม่มีทั้งคู่ → AI OCR ตามเดิมทุกประการ

**เงื่อนไขการถาม** (ครบทั้ง 3 ข้อเท่านั้น):

- รายการนั้น **ไม่ได้มาจากสลิป AI** อยู่แล้ว (`pending.slipToken` ว่าง)
- ผู้ใช้เป็น **Premium** — การแนบสลิปเป็นสิทธิ์ Premium
  (**ห้ามถาม Free แล้วปฏิเสธทีหลัง**)
- เปิด Session สำเร็จ (ถ้า DB ล่ม ไม่ถาม เพราะส่งรูปมาก็ไม่มีอะไรรับ)

> **TTL บังคับที่ชั้นอ่านเสมอ** (`findValidByUser` กรอง `updated_at` ทุกครั้ง)
> ไม่ใช่พึ่ง Cron ลบให้ — ต่อให้ Cron ไม่ทำงานเลย แถวที่ค้างก็ไม่มีผล รูปถัดไป
> ไหลเข้า AI OCR ตามปกติ (Cron เป็นแค่การเก็บกวาดพื้นที่)

> ⚠️ **ข้อความห้ามอ้างเรื่องภาษี** — ห้ามเขียนทำนอง "เก็บไว้ยื่นภาษี" /
...
**Version:** 1.0.0 | **Last Updated:** 22 สิงหาคม 2569
