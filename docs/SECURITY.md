...
5. **ระยะเวลาดำเนินการ** — **ทันที** (ไม่ใช่ภายใน 30 วันแบบ Flow เดิมที่
   ...
   Admin)
6. **ขอบเขตข้อมูลที่ลบ/Anonymize** — ดูตารางสรุปด้านล่าง (§ 8.1)
...
### 8.1 ⭐ อะไรถูกล้าง อะไรถูกเก็บ และ **ทำไม**

> อัปเดต 27 ส.ค. 2569 — เพิ่ม `portfolios.name` / `brokers.name` เข้ารายการที่ต้องล้าง

| ข้อมูล | ตอน Erasure | เหตุผล |
|---|---|---|
| `users.line_user_id` / `display_name` / `picture_url` | **Anonymize** | Field ระบุตัวตนโดยตรง (ยืนยันจาก Schema จริงแล้วว่าครบทุกตัวที่มี) |
| รูปสลิปใน Storage (`payment-slips` · `transaction-slips` · Facebook proof) | **Hard Delete** | เป็นภาพที่อาจมีชื่อ-เลขบัญชีจริง และไม่ใช่ Ledger ทางบัญชี |
| **`portfolios.name`** | **Anonymize → `พอร์ต <8 ตัวแรกของ id>`** | ผู้ใช้พิมพ์เอง อาจมี PII จริง (เช่น *"พอร์ตของสมชาย"*) และเป็น **ป้ายกำกับล้วน ไม่เข้าสูตรคำนวณเงินสักสูตร** |
| **`brokers.name`** | **Anonymize → `โบรก <8 ตัวแรกของ id>`** | เหตุผลเดียวกัน |
| `transactions` / `assets` / `payments` แถวจริง | **เก็บไว้ครบ ไม่แตะแม้แถวเดียว** | Immutable Ledger (PROJECT_BRIEF § 9) — เป็นหลักฐานทางบัญชีที่กฎหมายไทยกำหนดให้เก็บ (~5 ปี) และเป็นฐานของยอดเงินที่ผู้ใช้เคยตกลงไว้ |

> ### ⚠️ ทำไม `portfolios`/`brokers` **ไม่ได้รับเกราะ Immutable Ledger**
>
> เพราะมันเป็น **ป้ายกำกับ** ไม่ใช่ตัวเลข — **ลบชื่อทิ้งแล้วตัวเลขทุกตัวในระบบ
> ยังเท่าเดิมเป๊ะ** ต่างจาก `transactions` ที่ลบแล้วยอดเปลี่ยนทันที
> (มติ Founder 27 ส.ค. 2569)

> ### ⚠️ **Anonymize ไม่ใช่ DELETE** — และห้ามเปลี่ยนเป็น DELETE
>
> ลบแถว `portfolios`/`brokers` จะทำให้ `assets.portfolio_id`/`broker_id` กลายเป็น
> `NULL` (FK `ON DELETE SET NULL`) ซึ่งเท่ากับ **แก้ข้อมูลการลงทุนของผู้ใช้**
> และในกรณีของพอร์ตยัง **ละเมิด Invariant ของ migration 044/045** โดยตรง
> (สินทรัพย์ทุกแถวต้องสังกัดพอร์ต)

> ### 🔴 กับดักที่ต้องระวังถ้าจะแก้โค้ดส่วนนี้
>
> `brokers` มี **`uniq_brokers_user_name_ci ON (user_id, lower(name))`**
> (migration 042) — ถ้าเปลี่ยนชื่อโบรกทุกตัวเป็นค่าเดียวกัน ผู้ใช้ที่มีหลายโบรก
> จะ **ชน UNIQUE → Erasure ล้มทั้งก้อน** แล้วคนที่ยื่นคำขอลบข้อมูลจะลบไม่ได้เลย
> จึงต้องมีตัวแยกต่อท้ายชื่อเสมอ
> (Regression: `tests/erasureAnonymizeLabels.regression.test.js`)
>
> `portfolios` **ไม่มี** UNIQUE บนชื่อ (ตรวจ Schema จริงแล้ว) แต่ใส่ตัวแยกไว้
> เหมือนกันเพื่อกันไว้เผื่อวันหนึ่งมีคนเพิ่ม

### 8.2 📌 Open Question — `transactions.note` (ยังไม่มีใครตัดสิน)

> **ต้องมีคนตัดสินก่อนมีผู้ใช้จริงยื่นคำขอลบข้อมูล**

`transactions.note` เป็น **ข้อความอิสระที่ผู้ใช้พิมพ์เอง** เหมือน
`portfolios.name`/`brokers.name` ทุกประการ — การอ้าง "Immutable Ledger" เพื่อเก็บ
ไว้หลังคำขอลบตาม PDPA จึงเป็นฐานที่ **อ่อนกว่าที่คิด**: ตัวเลขในธุรกรรม
(จำนวน/ราคา/วันที่) คือสิ่งที่กฎหมายบัญชีต้องการ ส่วน `note` ไม่ใช่

ทางเลือกที่เป็นไปได้ (ยังไม่เลือก):

1. ล้าง `note` ตอน Erasure โดยไม่แตะตัวเลข — Ledger ยังครบทางบัญชี
2. เก็บไว้ตามเดิม แล้วระบุใน Privacy Policy ให้ชัดว่า `note` ถูกเก็บต่อ
3. ห้ามใส่ PII ใน `note` ตั้งแต่ต้นทาง (เตือนใน UI)

⚠️ **ห้ามแก้พฤติกรรมของ `note` เองโดยไม่มีมติ** — เป็นคำถามเชิงนโยบาย/กฎหมาย
ไม่ใช่บั๊ก และการล้าง Ledger ผิดพลาดย้อนกลับไม่ได้
