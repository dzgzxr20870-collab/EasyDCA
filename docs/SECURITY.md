...
### Flow การรับคำขอลบข้อมูล

1. **ช่องทางรับคำขอ** — ผู้ใช้ส่งคำขอผ่าน LINE OA (เช่น พิมพ์คำสั่ง
   "ลบข้อมูล") หรือผ่านอีเมล/ช่องทาง Support ที่ระบุใน Privacy Policy
2. **ยืนยันตัวตน** — ต้องยืนยันว่าผู้ร้องขอเป็นเจ้าของบัญชีจริง (ผ่าน
   LINE Login/LIFF หรือกระบวนการยืนยันตัวตนที่ Admin กำหนด) ก่อนดำเนินการ
   ใดๆ เพื่อป้องกันการปลอมคำขอลบข้อมูลของผู้อื่น
3. **แจ้งผลกระทบก่อนดำเนินการ** — แจ้งผู้ใช้ว่าการลบข้อมูลไม่สามารถ
   ย้อนกลับได้ และหากมี Premium ที่จ่ายเงินแล้วจะไม่ได้รับการคืนเงิน
   ให้ผู้ใช้ยืนยันอีกครั้งก่อนดำเนินการจริง
4. **Admin ดำเนินการลบ** — Role `Super Admin` หรือ `Admin` เท่านั้นที่มี
   สิทธิ์กดยืนยันการลบข้อมูลจริงใน Admin Dashboard (Phase 3)
5. **ระยะเวลาดำเนินการ** — ดำเนินการให้แล้วเสร็จภายใน **30 วัน**
   นับจากวันที่ยืนยันตัวตนสำเร็จ (ตามกรอบเวลาที่ PDPA กำหนด)
6. **ขอบเขตข้อมูลที่ลบ** — ลบข้อมูลระบุตัวตน (Hard Delete): LINE
   Profile, ประวัติธุรกรรม, รูปสลิป, เป้าหมาย, การตั้งค่า และ Watchlist
   ของผู้ใช้รายนั้นทั้งหมด
7. **ข้อมูลที่ต้องเก็บไว้ต่อแม้ถูกร้องขอลบ** — ข้อมูลที่กฎหมายอื่นบังคับ
   ให้เก็บ (เช่น หลักฐานทางบัญชี/ภาษีที่เกี่ยวกับการชำระเงิน) ให้เก็บแบบ
   Anonymized (ตัดการเชื่อมโยงกับตัวบุคคลออก) ตามระยะเวลาที่กฎหมายไทย
   กำหนด (เช่น เอกสารบัญชี ~5 ปี)
8. **ข้อยกเว้นชั่วคราว** — หากบัญชีมีข้อพิพาทการชำระเงินที่ยังไม่ยุติ
   (เช่น Payment อยู่ระหว่าง Reviewing/Dispute) ให้แจ้งผู้ใช้ว่าจะ
   ดำเนินการลบหลังข้อพิพาทยุติแล้ว
9. **บันทึก Audit** — ทุกคำขอและการดำเนินการลบข้อมูลต้องบันทึกลง
   `audit_logs` (`action = 'delete_user_data'`) พร้อมระบุ Admin ผู้ดำเนินการ
10. **แจ้งผลกลับผู้ใช้** — ส่งข้อความยืนยันว่าข้อมูลถูกลบเรียบร้อยแล้ว
    ผ่านช่องทางที่ผู้ใช้ร้องขอมา (ก่อนที่ LINE User ID จะถูกลบ)
...
| Critical Error (เช่น Approve Payment แล้ว Unlock Premium ล้มเหลว, Database Connection ล้มเหลวหลัง Retry ครบ, Cron Job ทั้งหมดล้มเหลว) | บันทึก `system_log` **และ** แจ้ง Admin ทันทีผ่าน LINE Notify |
...
- ระหว่างที่ยังไม่มี Admin Dashboard (Phase 0.5–2) แนะนำใช้ Uptime
  Monitoring ภายนอกแบบ Free Tier (เช่น UptimeRobot) Ping Endpoint หลัก
  เป็นระยะ เพื่อรู้ทันทีถ้า Railway หรือ API ล่ม ก่อนที่ Admin Dashboard
  จะพร้อมใช้งานจริง

### 9.4 Alert Channel

- ช่องทางแจ้งเตือนหลักสำหรับ Admin คือ **LINE Notify** (ตรงกับ Tech
  Stack ที่ตกลงไว้) ใช้แจ้งทั้งกรณี Critical Error และสลิปการชำระเงิน
  เข้าใหม่
