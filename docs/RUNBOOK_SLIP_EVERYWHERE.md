...
| A6 | บันทึกผ่านฟอร์มเว็บแบบกรอกจำนวนเงินเอง (ไม่กรอกราคา) | ยอดที่บันทึก = ยอดที่กรอก (พฤติกรรมเดิม) |

### A-web. บั๊ค A ทางเข้าที่ 3 — ฟอร์มเว็บ "จำนวนเงิน + ราคาที่กรอกเอง"

บั๊คตัวเดียวกับ A1-A3 คนละประตู (Founder สั่งรวมรอบนี้) — **ต้องกรอกราคาเองด้วย**
ถ้าเว้นช่องราคาว่างจะไปเข้าเส้นทาง A6 ซึ่งเป็นคนละ Branch

| ขั้นตอน | ทำอะไร | คาดหวัง |
|---|---|---|
| W1 | `/dashboard` → บันทึกรายการ → เลือก **BTC** → จำนวนเงิน `100` → ราคาต่อหน่วย `2513380` → บันทึก | การ์ดผลลัพธ์แสดง **100 บาท** (ก่อนแก้ได้ 100.01) |
| W2 | ดูแถวล่าสุดในประวัติรายการ | ยอด **100** ถ้วน |
| W3 | เลือกหุ้นไทย → จำนวนเงิน `1000` → ราคา `34` → บันทึก | ยอด **1,000** (เส้นทางเดิม ต้องไม่เปลี่ยน) |

> ⚠️ **ต้องใช้ราคาต่อหน่วยสูง (หลักล้าน) ถึงจะเห็นความต่าง** — หุ้นไทยราคาหลักสิบ/
> หลักร้อย เศษที่คูณกลับต่ำกว่า 0.005 อยู่แล้ว ปัดกลับได้ยอดเดิมเป๊ะทั้งก่อนและหลังแก้
