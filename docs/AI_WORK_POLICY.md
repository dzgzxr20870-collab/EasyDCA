...
#### ⭐ Checklist: RPC Function ที่ใช้ `RETURNS TABLE` (เพิ่ม 29 ส.ค. 2569)

> เพิ่มหลังเคสจริง `set_default_portfolio_locked` (migration 048) — ปุ่ม "ตั้งเป็น
> พอร์ตหลัก" ตอบ `500` ทุกครั้ง เพราะ `42702 column reference "user_id" is ambiguous`
> (ดู [`DECISIONS_LOG.md` — 29 ส.ค. 2569](./DECISIONS_LOG.md))

ใน PL/pgSQL ชื่อคอลัมน์ที่ประกาศใน `RETURNS TABLE (...)` จะกลายเป็น **ตัวแปร OUT
ที่มองเห็นได้ทั้งฟังก์ชันโดยอัตโนมัติ** ถ้าชื่อนั้นตรงกับชื่อคอลัมน์จริงของตารางที่
ฟังก์ชันเขียน/อ่านด้วย Postgres จะแยกไม่ออกว่าหมายถึงอันไหน แล้วโยน `42702` ทันที

- [ ] ถ้า `RETURNS TABLE (...)` ใช้ชื่อเดียวกับคอลัมน์จริงของตาราง → **ต้องใส่
      Table Alias ทุกจุดที่อ้างคอลัมน์ภายในฟังก์ชัน ห้ามอ้างชื่อเปล่าๆ แม้แต่จุดเดียว**
      (`UPDATE public.portfolios pf SET ... WHERE pf.user_id = p_user_id`
      ไม่ใช่ `WHERE user_id = p_user_id`)
- [ ] ตรวจให้ครบ **ทุก** Statement ในฟังก์ชัน ไม่ใช่แค่ตัวแรก — เคสจริงมี `UPDATE`
      2 ตัวและพังทั้งคู่
- [ ] `WHERE` / `SET` / `RETURNING` / `ON CONFLICT` นับหมด — ที่ไม่กำกวมคือ
      INSERT column list เท่านั้น
- [ ] **ต้องเรียกใช้จริงอย่างน้อย 1 ครั้งหลัง Apply** ห้ามถือว่า "Apply ผ่าน =
      ใช้งานได้" — `CREATE FUNCTION` ตรวจแค่ Syntax ไม่ได้ Resolve ชื่อคอลัมน์ให้
      จนกว่าจะรันจริง บั๊กตัวนี้จึงซ่อนอยู่ตั้งแต่ Apply 048 จนถึงวันที่มีคนกดปุ่ม
      เป็นครั้งแรก

⚠️ Function พี่น้องที่ยังไม่พังแต่มี Pattern เดียวกัน ต้องระวังเมื่อแก้ในอนาคต:
`create_portfolio_locked` (migration 048) — ตอนนี้รอดเพราะทุกจุดมี Alias (`p.user_id`)
หรืออยู่ในบริบทที่ไม่กำกวมอยู่แล้ว **ไม่ใช่เพราะออกแบบมากันไว้**
