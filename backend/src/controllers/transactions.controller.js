...
const WEB_ERROR_MESSAGES = {
  ...
  // Client ยิงซ้ำพร้อม confirmSeparatePortfolio ตามที่ผู้ใช้เลือก · ข้อความนี้เป็น
  // Fallback สำหรับ Client ที่ไม่ได้ทำ Dialog (เช่นเรียก API ตรง) — เว็บจะประกอบ
  // ประโยคเองจาก details ที่แนบไปให้ (มีชื่อพอร์ตจริงทั้งสองฝั่ง)
  ASSET_EXISTS_IN_OTHER_PORTFOLIO:
    'คุณถือสินทรัพย์นี้อยู่ในพอร์ตอื่นแล้ว — ยืนยันอีกครั้งว่าต้องการแยกเป็นรายการใหม่ในพอร์ตที่เลือก หรือรวมเข้าพอร์ตเดิม',
  ...
  // (ผู้ใช้ที่เคยเป็น Premium แล้วดาวน์เกรด) — กติกาเดียวกันอธิบายได้ทั้งคู่
  // ⚠️ ต้องบอก "ทางออกที่ยังทำได้จริง" ให้ครบ (Pattern เดียวกับ PORTFOLIO_READ_ONLY)
  // ไม่ใช่บอกแค่ว่าทำไม่ได้ — ไม่งั้นผู้ใช้จะเข้าใจว่าข้อมูลถูกล็อก/ถูกลบทั้งหมด
  // แล้วไม่บันทึกการขายที่เกิดขึ้นจริง → ยอดในพอร์ตผิดถาวร
  ASSET_LIMIT_REACHED:
  ...
  // ⚠️ ใช้คำว่า "ย้อน" ไม่ใช่ "ยกเลิก" — รายการนี้บันทึกลง Ledger ไปแล้วจริง (สร้าง
  // Reversal หักล้าง) คนละสถานะกับ Pending ที่ยังไม่บันทึก (มติ Founder: ห้ามใช้
  // คำเดียวกันสองความหมาย)
  NO_TRANSACTION_TO_UNDO: 'ไม่มีรายการให้ย้อน',
  ALREADY_UNDONE: 'รายการล่าสุดถูกย้อนไปแล้ว',
  CANNOT_UNDO_QUANTITY_MISMATCH:
    'ย้อนรายการนี้ไม่ได้ เพราะยอดคงเหลือปัจจุบันน้อยกว่าจำนวนที่ซื้อไว้ (มีการขายเกิดขึ้นหลังจากนั้น)',
  ...
  CANNOT_ATTACH_TO_REVERSAL: 'แนบสลิปให้รายการย้อนไม่ได้ — แนบได้เฉพาะรายการซื้อ/ขายจริง',
...
const ERROR_STATUS = {
  ...
  // 409 = "ขัดกับสถานะข้อมูลปัจจุบัน ต้องให้ผู้ใช้ตัดสินก่อน" (Pattern เดียวกับ
  // AMBIGUOUS_* ข้างบน) ไม่ใช่ 400 เพราะคำขอไม่ได้ผิดรูป — แค่ยังไม่ครบพอจะตอบได้
  ASSET_EXISTS_IN_OTHER_PORTFOLIO: 409,
...
async function createTransaction(req, res) {
  ...
  const params = {
    ...
    // ⭐ ผู้ใช้ยืนยันแล้วว่าต้องการ "แยกเป็นอีกแถวในพอร์ตที่เลือก" ทั้งที่ถือ Symbol
    // นี้อยู่ในพอร์ตอื่น (มติ Founder 29 ส.ค. 2569)
    //
    // ⚠️ รับเฉพาะ `true` แท้ๆ ไม่รับ Truthy อื่น ('false'/1/'yes') — Pattern เดียว
    // กับ sellAll ด้านบน · การแยกแถวกระทบต้นทุนเฉลี่ยที่ผู้ใช้จะเห็นต่อไป จึงต้อง
    // มาจากการกดยืนยันจริงเท่านั้น ห้ามให้ค่าผิดชนิดกลายเป็น "ยืนยัน" โดยบังเอิญ
    // ⚠️ ส่งต่อเฉพาะเมื่อเป็น boolean แท้ๆ — ต้องแยก `false` ("ผู้ใช้ตอบว่ารวม")
    // ออกจาก "ไม่ส่งมาเลย" ("ยังไม่ได้ถาม") ให้ขาด ไม่งั้นคนที่เลือก "รวมพอร์ต
    // เดิม" จะโดนถามซ้ำไม่รู้จบ · ค่าผิดชนิด ('true'/1) ถูกเมินทิ้ง = ยังไม่ตอบ
    ...(typeof body.confirmSeparatePortfolio === 'boolean'
      ? { confirmSeparatePortfolio: body.confirmSeparatePortfolio }
      : {}),
