...
const ERROR_MESSAGES = {
  ...
  // แล้วเพิ่มตัวใหม่ไม่ได้" และ "ตัวนี้ไม่ใช่ 1 ใน 2 ตัวแรกที่ยังซื้อเพิ่มได้"
  // (ผู้ใช้ที่เคยเป็น Premium แล้วดาวน์เกรด) — ต้องตรงกับฝั่ง Web
  // (transactions.controller.WEB_ERROR_MESSAGES) เป๊ะ คนละ Channel แต่ผู้ใช้ควร
  // เห็นคำอธิบายเดียวกัน · ต้องบอกทางออกที่ยังทำได้จริงให้ครบเสมอ (ห้ามให้เข้าใจว่า
  // ข้อมูลถูกลบ — กฎเหล็กข้อ 2)
  ASSET_LIMIT_REACHED:
  ...
  PORTFOLIO_READ_ONLY:
    'พอร์ตนี้เพิ่มรายการใหม่ไม่ได้ เพราะแพ็กเกจ Premium หมดอายุแล้วครับ แต่ยังบันทึกการขาย ยกเลิกรายการล่าสุด และย้ายสินทรัพย์ออกไปพอร์ตหลักได้ตามปกติ ข้อมูลเดิมอยู่ครบทุกรายการ ต่ออายุแล้วกลับมาเพิ่มรายการได้ทันทีครับ',
  ...
  //
  // ⚠️ ใช้คำว่า "ยกเลิก" ในข้อความตอบกลับกลุ่มนี้ (พรอมต์รวมคำ 30 ส.ค. 2569 —
  // Founder ต้องการคำเดียวที่เรียกฟีเจอร์นี้ทั้งเว็บ/LINE คือ "ยกเลิกรายการล่าสุด")
  // — กลับคำจากมติเดิมที่เคยบังคับใช้ "ย้อน" เพื่อไม่ให้ชนกับ "ยกเลิก" ที่ใช้กับ
  // Pending ที่ยังไม่เคยบันทึก (buildCancelledMessage ด้านล่าง) แต่คำว่า "ล่าสุด"/
  // "รายการนั้น" ที่ต่อท้ายทุกข้อความในกลุ่มนี้เป็นตัวแยกบริบทอยู่แล้ว ไม่จำเป็น
  // ต้องใช้คนละคำกับ Pending Cancel
...
// ตอบกลับคำสั่ง "ยกเลิกล่าสุด" (Command History) — แจ้งว่ายกเลิกรายการเดิมแล้ว
// โดยระบุว่าเป็นการสร้างรายการตรงข้ามชดเชย (ไม่ได้ลบของเดิม) ตาม DATABASE.md § 8
//
// ⚠️ Header ใช้คำว่า "ยกเลิกรายการล่าสุด" (พรอมต์รวมคำ 30 ส.ค. 2569 — Founder
// ต้องการคำเดียวที่เรียกฟีเจอร์นี้ทั้งเว็บ/LINE) — กลับคำจากมติเดิมที่เคยบังคับ
// ใช้ "ย้อน" แยกจาก buildCancelledMessage's "ยกเลิก" (Pending ที่ไม่เคยบันทึก)
// คำว่า "ล่าสุด" ที่ต่อท้ายในทุกจุดที่พูดถึงฟีเจอร์นี้เป็นตัวแยกบริบทอยู่แล้ว
// (buildCancelledMessage ไม่มีคำว่า "ล่าสุด" ต่อท้าย) จึงใช้ "ยกเลิก" ร่วมกันได้
//
...
// = ทุก type ที่ไม่ใช่ buy กลายเป็น "ขาย" → กด "ยกเลิกล่าสุด" บนรายการปันผลจะได้
// การ์ดที่เขียนว่า "...ก่อนบันทึกรายการ**ขาย**นี้" ทั้งที่ผู้ใช้เพิ่งยกเลิกรายการปันผล
//
...
// ปันผลนี้" (บริบท) ไม่ใช่จำนวนที่ถูกยกเลิกออกจากพอร์ต การโชว์เลขนั้นบนการ์ด
// "ยกเลิกรายการ" จะอ่านเป็น "ถูกหักออกจากพอร์ตเท่านี้" ทันที ซึ่งไม่จริงเลย
function buildUndoMessage(result) {
  ...
  return bubble({
    ...
    bodyContents: [
      ...
      // คำว่า "มูลค่ารวม" ใช้กับซื้อ/ขายเท่านั้น — ของปันผลคือ "เงินปันผลที่ยกเลิก"
      // (ไม่ใช่มูลค่าสินทรัพย์ที่ซื้อขาย) ใช้คำเดียวกันจะกำกวมทันที
      textLine(
        `${isDividend ? 'เงินปันผลที่ยกเลิก' : 'มูลค่ารวม'}: ${formatNumber(result.amountThb)} บาท`,
        {
          size: 'sm',
          color: COLOR.textSecondary,
        }
...
function buildHelpMessage() {
  const card = bubble({
    ...
    bodyContents: [
      ...
      // พรอมต์รวมคำ 30 ส.ค. 2569: การ์ดยืนยันเปลี่ยนกลับไปเขียนว่า "ยกเลิกรายการ
      // ล่าสุด" แล้ว (Founder ต้องการคำเดียวทั้งเว็บ/LINE) — สลับลำดับจาก
      // fix/undo-command-aliases เดิม (ที่เคยขึ้นนำด้วย "ย้อนล่าสุด" ตอนการ์ด
      // ยังพูดคำว่า "ย้อน") ให้ขึ้นนำด้วยคำหลักใหม่ก่อนเสมอ ส่วน "ย้อนล่าสุด" (คำ
      // เดิมก่อนหน้านั้นอีกที) ยังคง Parse ได้จริงเหมือนเดิม ระบุไว้เป็นทางเลือกคู่กัน
      // — ห้ามลบการรองรับคำพิมพ์เดิมทิ้ง (UNDO_LAST regex ใน commandParser.service.js
      // ไม่ถูกแตะ ยังรับครบทั้ง 4 คำ)
      textLine('• ยกเลิกล่าสุด (หรือ ย้อนล่าสุด) — ยกเลิกรายการซื้อ/ขายล่าสุด', {
        size: 'sm',
        color: COLOR.textSecondary,
      }),
