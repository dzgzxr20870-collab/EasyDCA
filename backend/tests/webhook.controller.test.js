...
describe('handleEvent — Image (แนบสลิปตอนแจ้งชำระ)', () => {
  ...
  // ไม่ถูก AI-OCR Premium Gate แตะเลย — แม้ผู้ใช้เป็น Free ก็ต้องอัปโหลดสลิปจ่ายเงินได้
  // (จำเป็นสำหรับทุกคนที่กำลังจะอัพเกรด) — คนละเรื่องกับ handleAssetSlipImage (AI อ่านสลิป
  // ธุรกรรม) ที่ Gate เฉพาะ Premium
  test('Free + มีคำขอ pending → สลิปจ่ายเงินอัปโหลดได้ตามปกติ (ไม่โดน OCR Premium Gate, ไม่เรียก OCR)', async () => {
    entitlement.isPremiumActive.mockReturnValue(false); // Free ชัดเจน
    paymentService.findPendingByUserId.mockResolvedValue({ id: 'pay-9', status: 'pending' });
    lineService.getMessageContent.mockResolvedValue({
      buffer: Buffer.from([7, 7, 7]),
      contentType: 'image/jpeg',
    });
    paymentService.hashSlipImage.mockReturnValueOnce('hash-free');
    paymentService.assertSlipNotReused.mockResolvedValueOnce(undefined);
    storageService.uploadPaymentSlip.mockResolvedValue('https://cdn.test/payment-slips/pay-9-1.jpg');
    paymentService.attachSlipImage.mockResolvedValue({ id: 'pay-9' });

    await handleEvent(imageEvent('img-free'));

    // เข้าทาง Payment Slip ปกติ — อัปโหลด + ผูกเข้าคำขอ
    expect(storageService.uploadPaymentSlip).toHaveBeenCalledWith('pay-9', Buffer.from([7, 7, 7]), 'image/jpeg');
    expect(paymentService.attachSlipImage).toHaveBeenCalled();
    // ไม่แตะ AI OCR เลย (Gate ของ handleAssetSlipImage ไม่ถูกเรียก) → ตอบ "ได้รับสลิป"
    // ตามปกติ ไม่ใช่การ์ดชวนอัพเกรดของ OCR
    expect(slipOcrService.extractSlip).not.toHaveBeenCalled();
    expect(lastReplyText()).toContain('ได้รับสลิป');
  });
...
// ═══════════════════════════════════════════════════════════════════════
// ⚠️ Pivot จาก LINE Chat Inline (พิมพ์ Trigger → ถามข้อความในแชทตรงๆ — Session-based)
// เป็นหน้าเว็บ /support แยกต่างหาก เพราะ Webhook ตอบอัตโนมัติชนกับตอน Admin เข้าไป
// ตอบมือใน LINE Chat Mode เดียวกัน (Bot ทับคำตอบของ Admin) — Flow ฝั่ง LINE เหลือ
// แค่พิมพ์ Trigger แล้วได้ Link ไปหน้าเว็บทันที ไม่มี Session/Rate Limit/Postback
...
describe('handleEvent — ติดต่อ Admin/Support', () => {
  ...
  // ⚠️ ต้องไม่มี Rate Limit/Session ใดๆ หลงเหลือฝั่ง LINE อีกต่อไป — พิมพ์ Trigger ซ้ำ
  // ต้องได้ Link เดิมทุกครั้ง (ตอบ Link คงที่ ไม่ Push/ไม่เขียน DB จึงไม่มีอะไรให้ Spam)
  test('พิมพ์ Trigger ซ้ำหลายครั้งติดกัน → ได้ Link เดิมทุกครั้ง ไม่มี Rate Limit', async () => {
    commandParser.parseCommand.mockReturnValue({ command: COMMANDS.CONTACT_SUPPORT, params: {} });

    await handleEvent(textEvent('ติดต่อแอดมิน'));
    const first = lastReplyText();
    await handleEvent(textEvent('ติดต่อแอดมิน'));
    const second = lastReplyText();

    expect(first).toContain('/support');
    expect(second).toContain('/support');
  });
});
