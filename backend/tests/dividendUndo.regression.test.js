...
describe('[2] การ์ด LINE ตอนย้อนปันผล ต้องไม่โกหกผู้ใช้', () => {
  ...
  // พรอมต์รวมคำ 30 ส.ค. 2569: label นี้เปลี่ยนจาก "เงินปันผลที่ย้อน" เป็น "เงินปันผล
  // ที่ยกเลิก" ตามคำหลักใหม่ของฟีเจอร์นี้ (Founder ต้องการคำเดียวทั้งเว็บ/LINE)
  test('ยอดเงินต้องใช้คำว่า "เงินปันผลที่ยกเลิก" ไม่ใช่ "มูลค่ารวม"', () => {
    const card = textOf(buildUndoMessage(DIVIDEND_RESULT));

    expect(card).toContain('เงินปันผลที่ยกเลิก');
