...
describe('commandParser.service', () => {
  ...
  // fix/undo-command-aliases: เพิ่ม "ย้อนล่าสุด"/"ย้อนรายการล่าสุด" หลังจาก
  // fix/misleading-messages (d89c2b6) เปลี่ยนคำที่ระบบพูดจาก "ยกเลิกรายการ" เป็น
  // "ย้อนรายการ" ทุกจุด (การ์ด LINE/ปุ่มเว็บ/ป้ายในประวัติ) — ถ้าไม่เพิ่ม ผู้ใช้จะ
  // พิมพ์ตามคำที่ระบบสอนเองแล้วระบบไม่รู้จัก คำเดิม "ยกเลิก..." ต้องยังใช้ได้
  // ครบ (Backward Compat — ผู้ใช้ที่คุ้นคำเดิมอยู่แล้วห้ามพัง)
  describe('UNDO_LAST — ยกเลิกล่าสุด / ย้อนล่าสุด / undo (ทั้ง 5 คำต้อง Parse เป็นคำสั่งเดียวกัน)', () => {
    test.each([
      'ยกเลิกล่าสุด',
      'ยกเลิกรายการล่าสุด',
      'ย้อนล่าสุด',
    ...
    // กัน Regex กว้างเกินจนไปกินคำสั่ง/คำที่ใช้ในบริบทอื่น — คำเดี่ยวๆ เหล่านี้
    // (ไม่มี "ล่าสุด" ต่อท้าย) ต้องไม่ถูกตีความเป็น UNDO_LAST เด็ดขาด เพราะ "ยกเลิก"
    // และ "ย้อน" ใช้ในความหมายอื่นทั่วระบบ (เช่น ปุ่ม ❌ ยกเลิก ของ Flow ต่างๆ,
    // "ย้อนรายการ" ในป้าย Note) — ถ้า Regex กินคำเหล่านี้ไปด้วยจะกลายเป็น Unknown
    // Command ที่ผู้ใช้ไม่ได้ตั้งใจพิมพ์
    test.each(['ยกเลิก', 'ย้อน', 'ล่าสุด', 'ย้อนกลับ', 'ยกเลิกรายการ'])(
      '"%s" ไม่ตีความเป็น UNDO_LAST (Regex ไม่กว้างเกิน)',
      (input) => {
        expect(parseCommand(input).command).not.toBe(COMMANDS.UNDO_LAST);
      }
    );
  });
