...
beforeEach(() => {
  ...
  // Multi-Currency (Round 10): Default FX เรตพร้อมใช้ (สำหรับ Preview ของธุรกรรม USD)
  // เคสที่ต้องการทดสอบ FX ล่มจะ Override เป็น null เอง
...
// ── Round 10: สกุลเงิน USD เก็บ "ตามจริง" (Native) ไม่แปลงเป็นบาทตอนบันทึก ──────
describe('สกุลเงิน USD (Round 10) — เก็บ USD ตามจริง ไม่แปลงตอนบันทึก', () => {
  ...
  test('processBuyCommand: 2 หุ้น × 300 USD → บันทึกเป็น USD ตามจริง (currency=USD, price 300, รวม 600)', async () => {
    ...
    // ⚠️ พฤติกรรมใหม่: ไม่แปลงเป็น THB — เก็บ 300 USD/หน่วย, รวม 600 USD, currency='USD'
    expect(transactionRepository.create).toHaveBeenCalledWith(
      expect.objectContaining({
        type: 'buy',
        quantity: 2,
  ...
  test('validateBuy คืน amounts.fx (ยอดเทียบบาทสำหรับ Preview เท่านั้น) ผ่าน fxRate.service', async () => {
    ...
    expect(amounts).toMatchObject({
      ...
      currency: 'USD',
      priceSource: 'user',
      // ยอดเทียบบาท (600 USD × 35 = 21000) ไว้แสดงผลเท่านั้น ไม่ถูกบันทึก
  ...
  test('ดึง FX Rate ไม่ได้ (null) → ยังบันทึก USD ได้ตามปกติ, fx=null (ไม่ Block)', async () => {
    ...
    // ต่างจากพฤติกรรมเดิม (Round 2) ที่ throw FX_RATE_UNAVAILABLE — ตอนนี้บันทึกได้
    expect(transactionRepository.create).toHaveBeenCalledWith(
      expect.objectContaining({ amountThb: 600, pricePerUnit: 300, currency: 'USD' })
    );
    expect(result).toMatchObject({ amountThb: 600, currency: 'USD' });
  });
  ...
  test('จำนวนเงินรวมเป็น USD ("ซื้อ MSFT 600 USD") → หาร quantity จากราคา USD (getCurrentPriceUsd)', async () => {
    ...
    // 600 USD / 300 USD/หุ้น = 2 หุ้น ; เก็บ currency=USD, ราคา 300 USD
    expect(transactionRepository.create).toHaveBeenCalledWith(
      expect.objectContaining({ quantity: 2, pricePerUnit: 300, amountThb: 600, currency: 'USD' })
    );
    expect(result).toMatchObject({ quantity: 2, currency: 'USD' });
  });
  ...
  test('จำนวนเงินรวม USD แต่ไม่มี USD Price Feed → PRICE_FEED_NOT_IMPLEMENTED (ไม่บันทึก)', async () => {
    ...
    await expect(
      processBuyCommand(USER_ID, { symbol: 'MSFT', amountThb: 600, currency: 'USD' })
    ).rejects.toMatchObject({ code: 'PRICE_FEED_NOT_IMPLEMENTED' });
  ...
  test('ไม่ระบุ USD (Default THB) → ไม่เรียก FX, บันทึก THB คงเดิม (Path เดิมไม่กระทบ)', async () => {
    ...
    // amounts ไม่มี key currency (Shape เดิม) → repository ได้ default 'THB'
    expect(transactionRepository.create).toHaveBeenCalledWith(
      expect.objectContaining({ pricePerUnit: 300, amountThb: 600, currency: 'THB' })
    );
    expect(result).toMatchObject({ pricePerUnit: 300, amountThb: 600, currency: 'THB' });
  });
});
...
describe('กองทุนรวมไทย (Round 7) — BUY Default ราคาจาก NAV (last_val)', () => {
  const FUND_ASSET = {
  ...
  test('(d) ซื้อกองทุนด้วยจำนวนเงิน (ไม่พิมพ์ราคา) → ใช้ last_val หาร quantity + priceSource secnav', async () => {
    ...
    const result = await processBuyCommand(USER_ID, {
      symbol: 'K-SELECT', amountThb: 1250, type: 'fund',
      projId: 'M0001', fundClassName: 'K-SELECT-A(A)',
    });

    expect(priceFeedService.getMutualFundNav).toHaveBeenCalledWith('M0001', 'K-SELECT-A(A)');
    // quantity = 1250 / 12.5 = 100
    expect(transactionRepository.create).toHaveBeenCalledWith(
      expect.objectContaining({ type: 'buy', quantity: 100, pricePerUnit: 12.5, amountThb: 1250 })
    );
    expect(result).toMatchObject({ quantity: 100, pricePerUnit: 12.5, priceSource: 'secnav' });
  });
  ...
  test('ซื้อกองทุน Asset ใหม่ → สร้าง Asset พร้อม proj_id + fund_class_name', async () => {
    ...
    await processBuyCommand(USER_ID, {
      symbol: 'K-SELECT', name: 'K Select Equity Fund', amountThb: 1000, type: 'fund',
      projId: 'M0001', fundClassName: 'K-SELECT-A(A)',
    });
  ...
  test('(f) ดึง NAV ไม่ได้ (feed throw) → MUTUAL_FUND_NAV_UNAVAILABLE, ไม่บันทึก', async () => {
    ...
    await expect(
      processBuyCommand(USER_ID, {
        symbol: 'K-SELECT', amountThb: 1000, type: 'fund',
        projId: 'M0001', fundClassName: 'K-SELECT-A(A)',
      })
    ).rejects.toMatchObject({ code: 'MUTUAL_FUND_NAV_UNAVAILABLE' });
    expect(transactionRepository.create).not.toHaveBeenCalled();
  });
  ...
  test('SEC ไม่ config → โยน SEC_NOT_CONFIGURED (แยกจาก NAV unavailable)', async () => {
    ...
    ).rejects.toMatchObject({ code: 'SEC_NOT_CONFIGURED' });
  });
  ...
  test('ผู้ใช้พิมพ์ราคาต้นทุนเอง → priceSource user, ไม่เรียก NAV, แต่ Asset เก็บ Class', async () => {
    ...
    const result = await processBuyCommand(USER_ID, {
      symbol: 'K-SELECT', name: 'K Select', quantity: 100, pricePerUnit: 12.34, type: 'fund',
      projId: 'M0001', fundClassName: 'K-SELECT-A(A)',
    });
  ...
  test('type=fund แต่ไม่มี projId (Manual fund) → ไม่เรียก SEC, ตกไป path ราคาปกติ (PRICE_FEED_NOT_IMPLEMENTED)', async () => {
    ...
    await expect(
      processBuyCommand(USER_ID, { symbol: 'XFUND', amountThb: 1000, type: 'fund' })
    ).rejects.toMatchObject({ code: 'PRICE_FEED_NOT_IMPLEMENTED' });
    expect(priceFeedService.getMutualFundNav).not.toHaveBeenCalled();
  });
});
